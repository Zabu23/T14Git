...
async function dokumentumokbeillesztese(){
try{
    ...
    const eredmenyek = [
      ...
      function ObjektumLetrehozo(adatok){
        let ujTomb = [];
            for(let i=0;i<adatok.length;i++){
                let daraboltSor = adatok[i].split(" ");
                let ujObjektum = {
                    Helyezés : Number(daraboltSor[0]),
                    CsapatMeret : Number(daraboltSor[1]),
                    SportAg : daraboltSor[2],
                    VersenySzam : daraboltSor[3]
                }
                ujTomb.push(ujObjektum)
            }
            return ujTomb
      }

      const Helsinki = ObjektumLetrehozo(eredmenyek)
      await collection.insertMany(Helsinki)
    console.log("A dokumentumok sikeresen be lettek illesztve!");
    client.close();
}
catch(err){
